...
def _rule_based_decision(score: int) -> dict:
    ...
    if score >= 80:
        strategy = "canary"
        reasoning = f"Risk score {score} is high — canary deployment recommended to limit blast radius."
    elif score >= 50:
        strategy = "rolling"
        reasoning = f"Risk score {score} is medium — rolling deployment recommended."
    else:
        strategy = None
        reasoning = f"Risk score {score} is low — standard deployment is fine."
